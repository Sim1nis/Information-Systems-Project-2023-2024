...
def PageRank(i,alpha):
    ...
    edges = ray.data.read_csv(f"hdfs://master:9000/user/hdoop/custom_graph/edges/graph_edges_{i+1}.txt",parse_options=csv.ParseOptions(delimiter=" ")).to_pandas().drop_duplicates()
    ...
    while True:
        ...
        """compute_deg = []
        for j in range(0,no_nodes,batch_size):
            compute_deg += ray.get([worker[i%actor_num].compute_x.remote(i) for i in range(j,min(j+batch_size,no_nodes))])
        """

...
def splitData(df, seed=42, num=3):
    np.random.seed(seed)
    perm = np.random.permutation(df.index)
    m = len(df.index)
    data = np.zeros(shape=(1, num), dtype=object)
    data_end = np.zeros(shape=(1, num-1), dtype=np.int_)
    if (num == 1):
        data[0][0] = df.iloc[:, :]
        return tuple(data)
...
def createNewCluster(reducers):
    cost = 0
    new_cluster = np.zeros(shape=(1,40))
...
class KMeansMapper(object):
    ...
    def assignCluster(self, method="mega_elkan", task_num=2):
...
class KMeansReducer(object):
    def __init__(self, value, *kmeansmappers):
        ...
        self._clusterAssment = None
        self._cost = 0
        self._clusterOutput = np.zeros((1,40))

...
with parallel_backend('ray'):
	...
	X_train = ray.data.read_parquet("hdfs://master:9000/user/hdoop/Regression/train/")
	...
	X_test = ray.data.read_parquet("hdfs://master:9000/user/hdoop/Regression/test/")
	...
	reg = RayXGBRegressor(objective='reg:squarederror',booster='gblinear',n_estimators=100,learning_rate=0.01,ray_params=RayParams(num_actors=3,cpus_per_actor=2))

	print("Training...")
	reg.fit(X_train_xgb,None,RayParams(num_actors=3,cpus_per_actor=2))

	print("Predicting...")
	preds = reg.predict(X_test_imp.select_columns([f"feat_{i+1}" for i in range(num_features)]).to_pandas(),ray_params=RayParams(num_actors=3,cpus_per_actor=2))

	rmse = root_mean_squared_error(preds,X_test_imp.to_pandas()["label"].to_numpy())
